...
class preprocess:
    ...
    def set_columns(self, data):
        ...
        try:
            data.columns = ["symboling", "normalized-losses", "make", "fuel-type", "aspiration", "num-of-doors",
                            "body-style", 'drive-wheels', 'engine-location', 'wheel-base', 'length', 'width', 'height',
                            'curb-weight', 'engine-type', 'num-of-cylinder', 'engine-size', 'fuel-system', 'bore',
    ...
    def target(self, data):
        """
                                ...
                                On Failure: Raise Exception .
        """
        self.logger.log(self.log_file, "This method will return the target variable further for model building ")
        try:
            self.logger.log(self.log_file, "Firstly preprocesing the target variable")
            data.dropna(subset=["price"], axis=0, inplace=True)
            self.logger.log(self.log_file,"Now setting indexes back to normal after droping missing values rows from target variable")
            data.reset_index(drop=True, inplace=True)
            self.logger.log(self.log_file, "target variable preprocessing done")
            return data
        except Exception as e:
            self.logger.log(self.log_file, "Target variable preprocessing not succesful")
            raise e
        ...
        try:
            ...
            useful_data = data[
                ["length", "width", 'horsepower', 'curb-weight', "engine-size", "city-mpg", "highway-mpg",
                 'drive-wheels', 'num-of-cylinder','price']]
    ...
    def imputation(self, data):
        ...
        try:
            ...
            num_col = data.select_dtypes(include=[np.number]).columns.drop('price')
    ...
    def feature_remove(self, data):
        ...
        try:
            ...
            arrange_data = data[
                ['horsepower', 'curb-weight', 'engine-size', 'drive-wheels', 'num-of-cylinder', 'miles', 'area','price']]
